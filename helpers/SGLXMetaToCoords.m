...
function res = SGLXMetaToCoords(metaFullPath, varargin)
...
switch outType
    ...
    case 1     %KS2 *.mat
        nchan = numel(xCoord);
        res.chanMap = (1:nchan)';
        res.chanMap0ind = res.chanMap - 1;
        res.connected = logical(connected);
        res.xcoords = shankInd*shankPitch + xCoord;   %KS2 not yet using kcoords, so x coord includes shank sep
        res.ycoords = yCoord; % variable names need to  match KS standard
        res.kcoords = shankInd + 1;     %KS1 uses kcoords to force templates to be on one shank
        if tosave
            newName = fullfile(pathsave, [fname,'_kilosortChanMap.mat']);
            save( newName, '-struct','res',"-v7");
        end
